...
import * as quoAction from '../../actions/solicitudes';
...
const quoServices = React.createClass({
  ...
  store(id) {
    ...
    let solicitudId = this.props.solicitudes.solicitud.id;
    this.props.dispatch( quoAction.storeService(solicitudId, service) )
  ...
  render() {
    ...
    const serviceNodes = this.props.solicitudes.services.map(service => 
      <tr key={service.id} className="solicitud-service">
        <td>{service.title}</td>
        <td>
          <button 

...
const createPanelSolicitudes = React.createClass({
  ...
  createQuotation(contact, e) {
    ...
    .then(res => page(`/solicitudes/${res.payload.id}`) );
  },
              ...
              <tbody>
                {this.props.contacts.items.map(contact =>
                  <tr key={contact.id}>
                    ...
                    <td>{contact.email}</td>
                    <td><button className="btn btn-primary btn-sm" onClick={this.createQuotation.bind(this, contact)}>Crear Solicitud</button></td>
                  </tr>
                )}
              </tbody>
            </table>
